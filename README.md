...
Spring Security Filter Chain<br/><br/>

기본적인 기능은 인증(적절한 사용자인지 확인하는 것)<br/>
두번째 확인 기능은 권한 부여(사용자가 적절한 액세스 권한을 보유했는지 확인하는 것)<br/><br/>

한 가지 예로, CORS 즉 오리진 간 리소스 공유를 설정할 수 있다.<br/><br/>

또 다른 기능은 CSRF, 사이트 간 요청 위조가 있다.<br/> 
사용자가 웹사이트에 로그인하면 사용자 세션이 생성되는데, 이 사용자 세션은 브라우저의 쿠키를 사용해 식별된다. 사용자가 이 웹사이트에서 로그아웃하지 않은 채 악성 웹사이트로 이동하면, 악성 웹사이트에서는 사용자의 이전 인증 정보(브라우저에 있는 쿠키)를 이용할 수 있다. 이러한 취약성을 CSRF라 한다.<br/><br/>  

Spring Security는 기본 로그인 페이지와 기본 로그아웃 페이지도 제공한다.<br/>

-> 이러한 필터가 특정한 순서대로 실행된다.<br/><br/>
...
1. 키 쌍이라는 것을 만든다.<br/>
2. RSA 객체라는 것을 만든다.<br/>
3. JWKSource, 즉 JSON Web Key 소스를 만들어야 한다.<br/>
4. 디코딩과 인코딩을 설정해야 한다.<br/>
